...
router.get('/', [
    ...
    (0, express_validator_1.query)('vehicleType').optional().isIn(['car', 'truck', 'motorcycle', 'bus', 'van', 'suv']),
    (0, express_validator_1.query)('status').optional().isIn(['active', 'parked', 'maintenance', 'alert']),
    ...
    try {
        ...
        if (search) {
            ...
            filteredVehicles = filteredVehicles.filter(vehicle => vehicle.plateNumber.toLowerCase().includes(searchTerm) ||
        ...
        const paginatedVehicles = filteredVehicles.slice(Number(offset), Number(offset) + Number(limit));
        return res.json({
            success: true,
...
router.post('/', [
    (0, express_validator_1.body)('plateNumber').notEmpty().isLength({ min: 1, max: 20 }),
    (0, express_validator_1.body)('vehicleType').isIn(['car', 'truck', 'motorcycle', 'bus', 'van', 'suv']),
    (0, express_validator_1.body)('make').notEmpty().isLength({ min: 1, max: 100 }),
    (0, express_validator_1.body)('model').notEmpty().isLength({ min: 1, max: 100 }),
    (0, express_validator_1.body)('year').isInt({ min: 1990, max: new Date().getFullYear() + 1 }),
    (0, express_validator_1.body)('color').optional().isLength({ max: 50 }),
    (0, express_validator_1.body)('vinNumber').notEmpty().isLength({ min: 17, max: 17 }),
    (0, express_validator_1.body)('status').isIn(['active', 'parked', 'maintenance', 'alert']),
    (0, express_validator_1.body)('department').notEmpty().isLength({ min: 1, max: 255 }),
    (0, express_validator_1.body)('departmentId').notEmpty(),
    (0, express_validator_1.body)('assignedEmployeeId').optional().isString(),
    handleValidation
], async (req, res) => {
    try {
...
router.put('/:id', [
    (0, express_validator_1.param)('id').notEmpty(),
    (0, express_validator_1.body)('plateNumber').optional().isLength({ min: 1, max: 20 }),
    (0, express_validator_1.body)('vehicleType').optional().isIn(['car', 'truck', 'motorcycle', 'bus', 'van', 'suv']),
    (0, express_validator_1.body)('status').optional().isIn(['active', 'parked', 'maintenance', 'alert']),
    (0, express_validator_1.body)('fuelLevel').optional().isInt({ min: 0, max: 100 }),
    (0, express_validator_1.body)('mileage').optional().isInt({ min: 0 }),
    handleValidation
], async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:id/assign-tracker', [
    (0, express_validator_1.param)('id').notEmpty(),
    (0, express_validator_1.body)('trackerId').notEmpty(),
    handleValidation
], async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:id/unassign-tracker', [
    ...
    try {
        ...
        delete updatedVehicle.gpsTracker;
        delete updatedVehicle.gpsTrackerId;
...
router.post('/:id/maintenance', [
    ...
    (0, express_validator_1.body)('mileage').isInt({ min: 0 }),
    ...
    (0, express_validator_1.body)('parts').optional().isArray(),
    handleValidation
], async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (vehicleIndex !== -1) {
            const updatedVehicle = { ...mockVehicles[vehicleIndex] };
            updatedVehicle.lastMaintenance = maintenanceData.date;
            updatedVehicle.updatedAt = new Date().toISOString();
            mockVehicles[vehicleIndex] = updatedVehicle;
        }
...
router.get('/:id/tracking-history', [
    (0, express_validator_1.param)('id').notEmpty(),
    (0, express_validator_1.query)('start').isISO8601(),
    (0, express_validator_1.query)('end').isISO8601(),
    handleValidation
], async (req, res) => {
    try {
        const { id } = req.params;
