...
class AuthService {
    ...
    async hashPassword(password) {
        try {
            const salt = crypto_1.default.randomBytes(ARGON2_CONFIG.saltLength);
    ...
    async checkRateLimit(identifier, action, maxAttempts = 5, windowMinutes = 15) {
    ...
    async completeLogin(user, ipAddress, userAgent) {
        // Update last login
    ...
    async generateTokens(user) {
        ...
        const jti = crypto_1.default.randomUUID(); // JWT ID for token revocation
        ...
        const accessToken = jsonwebtoken_1.default.sign(accessTokenPayload, this.JWT_SECRET, {
    ...
    async verifyWebAuthnRegistration(userId, credential) {
        const user = this.users.get(userId);
    ...
    async verifyTOTP(userId, token) {
        ...
        await this.auditLog('totp_verified', userId, userId, 'mfa', 'totp', 'verify', verified ? 'success' : 'failure', { token: token.substring(0, 2) + '****' });
        return verified;
    }
    ...
    hasDepartmentAccess(user, targetDepartmentId) {
    ...
    generateDeviceFingerprint(userAgent) {
        return crypto_1.default.createHash('sha256').update(userAgent).digest('hex');
    }
    // Audit logging (immutable)
    async auditLog(eventType, actorId, targetUserId, resourceType, resourceId, action, outcome, metadata = {}, ipAddress, userAgent) {
        const auditEvent = {
    ...
    calculateRiskScore(eventType, outcome, metadata) {
        let score = 0;
        if (outcome === 'failure')
            score += 20;
        if (eventType.includes('admin') || eventType.includes('privileged'))
            score += 30;
