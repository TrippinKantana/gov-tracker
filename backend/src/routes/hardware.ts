...
router.get('/bw32/devices', async (_req: express.Request, res: express.Response) => {
  try {
    return res.json({
      success: true,
...
router.post('/bw32/command/:id',
  [
    param('id').notEmpty(),
    body('command').isIn(['engine_cutoff', 'engine_restore', 'get_location', 'set_geofence', 'request_sos']),
...
router.post('/bw32/devices',
  [
    body('imei').isLength({ min: 15, max: 15 }).matches(/^\d+$/),
    body('simNumber').notEmpty(),
    handleValidation
  ],
  async (req: express.Request, res: express.Response) => {
    try {
...
router.put('/bw32/devices/:id',
  [
    ...
    body('signalStrength').optional().isInt({ min: 0, max: 100 }),
    handleValidation
  ],
  async (req: express.Request, res: express.Response) => {
    try {
...
router.get('/bw32/status', async (_req: express.Request, res: express.Response) => {
  try {
