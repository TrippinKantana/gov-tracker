/**
 * Edit Fleet Modal - Complete Structure
 * Matches Add Fleet dialog exactly for consistency
 */
...
const EditFleetModal = ({ isOpen, onClose, onSuccess, vehicleId }: EditFleetModalProps) => {
    console.log('🔧 EditFleetModal rendered - NEW VERSION with Personnel Assignment - isOpen:', isOpen, 'vehicleId:', vehicleId);
  ...
  const [showOperatorDropdown, setShowOperatorDropdown] = useState(false);
            ...
            try {
                ...
                if (macsResponse.ok) {
                    ...
                    if (macsData.success && macsData.departments) {
                        ...
                        console.log('✅ Loaded real MACs for edit dropdown:', macsData.departments.length);
                    }
                }
    ...
    // Fetch existing vehicle data when modal opens
    useEffect(() => {
        if (isOpen && vehicleId) {
            fetchVehicleData();
        }
        ...
        try {
            ...
            if (result.success && result.vehicle) {
                ...
                setFormData({
                    ...
                    maintenanceInterval: vehicle.maintenanceInterval || 5000,
                    notes: vehicle.notes || '',
                ...
                setGeneratedGSACode(vehicle.gsaCode || '');
            }
        } catch (error) {
    ...
    const handleManualCountSave = async () => {
        ...
        try {
            ...
            // Check if GSA code already exists (excluding current vehicle)
            const codeExists = await checkGSACodeExists(newGSACode, 'vehicle', vehicleId || undefined);
            if (codeExists) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        console.log('🔧 Edit Fleet form submitted');
                ...
                <div className="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-5xl">
                    ...
                    <form onSubmit={handleSubmit} className="px-6 py-6 max-h-[70vh] overflow-y-auto">
                        ...
                        <div className="space-y-8">
                            ...
                            {/* Footer - Inside Form */}
                            <div className="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-t border-gray-200 dark:border-gray-600 -mx-6 -mb-6 mt-8">
                                <div className="flex justify-end space-x-3">
                                    ...
                                    <button
                                        type="button"
                                        onClick={(e) => {
                                            console.log('🔧 Update Fleet button clicked!');
                                            handleSubmit(e);
                                        }}
                                        disabled={isLoading}
